#!/usr/bin/env python3
"""
Migration script to update tag_utils.py for NKBIP-01 compliance
This creates a new version that integrates the NKBIP-01 tag requirements
"""
...
def create_backup():
    """Create a backup of the original tag_utils.py"""
    source = "modules/tag_utils.py"
    backup = f"modules/tag_utils.py.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    shutil.copy2(source, backup)
    print(f"Created backup: {backup}")
    return backup

def create_updated_tag_utils():
    """Create the updated tag_utils.py with NKBIP-01 support"""
    content = '''import re
...
'''
    ...
    with open("modules/tag_utils.py", "w") as f:
        f.write(content)
    
    print("Updated modules/tag_utils.py with NKBIP-01 support")

def main():
    print("NKBIP-01 Tag Utils Migration")
    print("=" * 40)
    ...
    # Update tag_utils.py
    try:
        create_updated_tag_utils()
        print("\n✅ Migration successful!")
        print("\nNext steps:")
        print("1. Test the updated tag_utils.py")
        print("2. Update nip62_converter.py to use the new tags")
        print("3. Run validation tests")
        print(f"\nIf issues occur, restore from: {backup_file}")
    except Exception as e:
        print(f"\n❌ Migration failed: {e}")
